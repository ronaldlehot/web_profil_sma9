...
if($_FILES['foto']['error'] == 0){
	...
	$query_artikel = mysqli_query($koneksi, "SELECT foto FROM tbl_informasi WHERE id = $id");
...
$tujuan = '../../images/informasi/';
		
if($_FILES['foto']['error'] == 0){
	if($_FILES['foto']['size'] < 1000000){
		if (file_exists('../../images/informasi/' . $nama_foto)) unlink('../../images/informasi/' . $nama_foto . '.' . $ekstensi);
		if(file_exists('../../images/informasi/' . $row['foto'])) unlink('../../images/informasi/' . $row['foto']);
		move_uploaded_file($asal, $tujuan . $nama_foto) or die('gagal upload foto');

		// ubah data
		$query = mysqli_query($koneksi, "UPDATE tbl_informasi SET judul = '$judul', id_kategori = '$id_kategori', isi = '$isi', foto = '$nama_foto' WHERE id = $id") or die(mysqli_error($koneksi));
		if($query){
