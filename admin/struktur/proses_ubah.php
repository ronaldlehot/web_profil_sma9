...
if($_FILES['foto']['error'] == 0){
	...
	$query_artikel = mysqli_query($koneksi, "SELECT foto FROM tbl_struktur WHERE id = $id");
...
if($_FILES['foto']['error'] == 0){
	if($_FILES['foto']['size'] < 1000000){
		...
		// ubah data
		$query = mysqli_query($koneksi, "UPDATE tbl_struktur SET  foto = '$nama_foto' WHERE id = $id") or die(mysqli_error($koneksi));
		if($query){
